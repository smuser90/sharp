 ...
 */
var resize = function resize (width, height, options) {
  if (is.defined(width)) {
    if (is.integer(width) && is.inRange(width, 1, this.varructor.maximum.width)) {
      this.options.width = width;
    } else {
      throw is.invalidParameterError('width', `integer between 1 and ${this.varructor.maximum.width}`, width);
    }
  } else {
    this.options.width = -1;
  }
  if (is.defined(height)) {
    if (is.integer(height) && is.inRange(height, 1, this.varructor.maximum.height)) {
      this.options.height = height;
    } else {
      throw is.invalidParameterError('height', `integer between 1 and ${this.varructor.maximum.height}`, height);
    }
  } else {
 ...
 * @returns {Sharp}
 */
var ignoreAspectRatio = function ignoreAspectRatio () {
 ...
 * @returns {Sharp}
*/
var withoutEnlargement = function withoutEnlargement (withoutEnlargement) {
