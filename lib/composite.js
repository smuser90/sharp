 ...
 */
var overlayWith = function overlayWith (overlay, options) {
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.left) || is.defined(options.top)) {
      if (
        is.integer(options.left) && is.inRange(options.left, 0, this.varructor.maximum.width) &&
        is.integer(options.top) && is.inRange(options.top, 0, this.varructor.maximum.height)
      ) {
    ...
    if (is.defined(options.gravity)) {
      ...
      } else if (is.string(options.gravity) && is.integer(this.varructor.gravity[options.gravity])) {
        this.options.overlayGravity = this.varructor.gravity[options.gravity];
      } else {
